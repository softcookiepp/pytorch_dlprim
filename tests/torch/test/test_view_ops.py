...
class TestViewOps(TestCase):
	...
	def test_conj_self(self):
		for dtype in all_types_and(torch.half, torch.bfloat16):
			t = torch.ones(5, 5, device=self._device)
			s = t.conj()
			self.assertTrue(s is t)

	def test_view_dtype_new(self):
		...
		for dtype in all_types_and_complex_and(torch.half, torch.bool):
			def generate_inputs():
				yield make_tensor((4, 4, 64), dtype=dtype, device=self._device, low=-5, high=5)
				yield make_tensor(
					(4, 4, 64), dtype=dtype, device=self._device, low=-5, high=5
				).permute(1, 0, 2)
				yield make_tensor(
					(4, 64, 4), dtype=dtype, device=self._device, low=-5, high=5
				).permute(2, 0, 1)
				yield make_tensor(
					(1, 5, 1), dtype=dtype, device=self._device, low=-5, high=5
				).expand(5, 5, 64)
				yield make_tensor((2, 5, 256), dtype=dtype, device=self._device, low=-5, high=5)[
					1::2, 1:, ::2
				]
				yield make_tensor((0, 5, 64), dtype=dtype, device=self._device, low=-5, high=5)
				yield make_tensor((), dtype=dtype, device=self._device, low=-5, high=5)
	...
	def test_view_dtype_upsize_errors(self):
		for dtype in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool):
			...
			for view_dtype in all_types_and_complex_and(
				...
				size_ratio = view_dtype_size // dtype_size
				a = make_tensor(
					(4, 4, size_ratio + 1), dtype=dtype, device=self._device, low=-5, high=5
				)
				with self.assertRaisesRegex(
				...
				a = make_tensor(
					(4, 4, size_ratio), dtype=dtype, device=self._device, low=-5, high=5
				)
	...
	def test_view_as_complex(self):
		def fn(contiguous_input=True, dim0=0, dim1=1):
			t = torch.randn(3, 2, 2, device=self._device)
			c_t = t[:, :, 0] + 1j * t[:, :, 1]
		...
		x = torch.randn(3, 3, device=self._device)
		...
		x = torch.tensor([], device=self._device)  # torch.Size([0])
		self.assertRaisesRegex(
			RuntimeError,
			"Tensor must have a last dimension of size 2",
		...
		x = torch.rand(336, 2, 1, device=self._device)
		...
		# sparse matrix
		x = torch.tensor([[2.0, 3.0], [4.0, 5.0]], device=self._device)
		indices = torch.tensor([[0, 2], [0, 1]], device=self._device)
	...
	def test_view_as_real(self):
		for dtype in [complex_types(), torch.complex32]:
			def fn(contiguous_input=True):
				t = torch.randn(3, 4, dtype=dtype, device=self._device)
			...
			# tensor with zero elements
			x = torch.tensor([], dtype=dtype, device=self._device)
			res = torch.view_as_real(x)
			self.assertTrue(self.is_view_of(x, res))
			self.assertEqual(res.shape, torch.Size([0, 2]))

			# tensor with zero dim
			x = torch.tensor(2 + 3j, dtype=dtype, device=self._device)
			res = torch.view_as_real(x)
			self.assertTrue(self.is_view_of(x, res))
			...
			# sparse matrix
			x = torch.tensor(2 + 3j, dtype=dtype, device=self._device)
			indices = torch.tensor([[0], [0]], device=self._device)
			size = torch.Size([3, 3])
	...
	def test_view_tensor_split(self):
		for dtype in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool):
			a = make_tensor((40, 30), dtype=dtype, device=self._device, low=-9, high=9)
	...
	def test_view_tensor_hsplit(self):
		for dtype in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool):
	...
	def test_imag_noncomplex(self):
		for dtype in all_types_and(torch.half, torch.bfloat16):
			t = torch.ones((5, 5), dtype=dtype, device=self._device)

			with self.assertRaises(RuntimeError):
				torch.imag(t)

	def test_real_imag_view(self):
		for dtype in complex_types():
			def compare_with_numpy(contiguous_input=True):
				t = torch.randn(3, 3, dtype=dtype, device=self._device)
				...
				re = u.real
				exp = torch.from_numpy(u.cpu().numpy().real).to(device=self._device)
				...
				im = u.imag
				exp = torch.from_numpy(u.cpu().numpy().imag).to(device=self._device)
	...
	def test_conj_imag_view(self) -> None:
		for dtype in complex_types():
			t = _make_tensor((4, 5), dtype, device)
			t_numpy_conj = torch.from_numpy(t.cpu().numpy().conj()).to(device=self._device)
			v = t.conj()
	...
	#@onlyNativeDeviceTypes
	def test_conj_view_with_shared_memory(self) -> None:
	...
	@suppress_warnings
	def test_set_real_imag(self, dtypes):
		x = torch.randn(10, dtype=dtypes[0], device=self._device)
	...
	def test_diagonal_view(self) -> None:
		...
		t = torch.ones((3, 3, 3), device=self._device)
	...
	@skipLazy
	def test_unbind_view(self) -> None:
		t = torch.zeros((5, 5), device=self._device)
		tup = torch.unbind(t)
	...
	@skipLazy
	def test_expand_view(self) -> None:
		t = torch.ones((5, 1), device=self._device)
		v = t.expand(5, 5)
		self.assertTrue(self.is_view_of(t, v))
	...
	def test_expand_as_view(self):
		t = torch.ones((5, 1), device=self._device)
		e = torch.empty((5, 5), device=self._device)
		v = t.expand_as(e)
		self.assertTrue(self.is_view_of(t, v))
	...
	def test_permute_view(self) -> None:
		t = torch.ones((5, 5), device=self._device)
		v = t.permute(1, 0)
		self.assertTrue(self.is_view_of(t, v))
	...
	def test_transpose_inplace_view(self):
		t = torch.ones(5, 5, device=self._device)
		v = t.view_as(t)
		v = v.swapdims_(0, 1)
		self.assertTrue(self.is_view_of(t, v))
	...
	def test_unfold_view(self):
		t = torch.ones(10, device=self._device)
		v = t.unfold(0, 3, 2)
		self.assertTrue(self.is_view_of(t, v))
	...
	def test_squeeze_view(self):
		t = torch.ones(5, 1, 5, device=self._device)
		v = torch.squeeze(t)
		self.assertTrue(self.is_view_of(t, v))
		v[0, 1] = 0
		self.assertEqual(t, v._base)

	def test_squeeze_inplace_view(self):
		t = torch.ones(5, 5, device=self._device)
		v = t.view_as(t)
		v = v.squeeze_()
		self.assertTrue(self.is_view_of(t, v))
	...
	def test_unsqueeze_inplace_view(self):
		t = torch.ones(5, 5, device=self._device)
		v = t.view_as(t)
		v = v.unsqueeze_(1)
		self.assertTrue(self.is_view_of(t, v))
	...
	@onlyCUDA
	def test_as_strided_inplace_view(self):
		t = torch.ones(5, 5, device=self._device)
		v = t.view_as(t)
	...
	@skipLazy
	def test_contiguous_nonview(self):
		t = torch.ones(5, 5, device=self._device)
		nv = t.t().contiguous()
	...
	def test_reshape_as_view(self):
		t = torch.ones(5, 5, device=self._device)
		e = torch.empty((25,), device=self._device)
		v = t.reshape_as(e)
		self.assertTrue(self.is_view_of(t, v))
	...
	def test_flatten_view(self):
		...
		t = torch.ones(1, 2, 3, 4, device=self._device)
		v = t.flatten()
		self.assertTrue(self.is_view_of(t, v))
		test_writes_propagate(t, v)

		# zero-dimensional tensor
		t = torch.tensor(1, device=self._device)
		v = t.flatten()
		test_writes_propagate(t, v)
		self.assertTrue(self.is_view_of(t, v))

		t = torch.ones(1, 2, 3, 4, device=self._device).transpose(2, 3)
		v = t.flatten(0, 1)
		test_writes_propagate(t, v)
		self.assertTrue(self.is_view_of_same_base(t, v))

		# stride[i] = stride[i + 1] * size[i + 1] is satisfied for 3 groups:
		t = torch.ones(720, device=self._device).as_strided(
	...
	def test_flatten_nonview(self):
		...
		t = torch.ones(2, 3, 2, 3, device=self._device).transpose(2, 3)
		nv = t.flatten(1, 3)
		assert_is_nonview(t, nv)

		t = torch.ones(2, 2, device=self._device).T
		nv = t.flatten()
		assert_is_nonview(t, nv)

		# flatten returns the original object if start_dim=end_dim
		t = torch.ones(2, 2, device=self._device)
		nv = t.flatten(1, 1)
		self.assertTrue(t is nv)

	def test_basic_indexing_slice_view(self):
		t = torch.ones(5, 5, device=self._device)
		v = t[:2, :3]
		self.assertTrue(self.is_view_of(t, v))
	...
	def test_basic_indexing_ellipses_view(self):
		t = torch.ones(5, 5, device=self._device)
		v = t[..., :2]
		self.assertTrue(self.is_view_of(t, v))
	...
	def test_basic_indexing_newaxis_view(self):
		t = torch.ones(5, 5, device=self._device)
		v = t[None, :2, 3]
		self.assertTrue(self.is_view_of(t, v))
	...
	def test_advanced_indexing_nonview(self):
		t = torch.ones(3, 3, device=self._device)
		rows = torch.tensor([[0, 0], [2, 2]], device=self._device)
		cols = torch.tensor([[0, 1], [2, 2]], device=self._device)
		nv = t[rows, cols]
		self.assertTrue(not self.is_view_of(t, nv))
	...
	def test_advanced_indexing_assignment(self):
		t = torch.ones(3, 3, device=self._device)
		rows = torch.tensor([[0, 0], [2, 2]], device=self._device)
		cols = torch.tensor([[0, 1], [2, 2]], device=self._device)
		t[rows, cols] = 0
	...
	def test_view_copy(self):
		a = torch.randn(4, device=self._device, requires_grad=True)
	...
	def test_view_copy_output_contiguous(self):
		a = torch.randn(4, 4, 4, 4, device=self._device).to(memory_format=torch.channels_last)
		b = torch.ops.aten.slice_copy(a, 0, 0, 2)
		self.assertTrue(b.is_contiguous())

	def test_view_copy_out(self):
		a = torch.randn(2, 2, device=self._device)
		out = torch.empty(2, device=self._device)
		...
		a = torch.randn(4, device=self._device)
		out1 = torch.empty(2, device=self._device)
		out2 = torch.empty(2, device=self._device)
	...
	def test_maybe_view_chunk_cat(self):
...
class TestOldViewOps(TestCase):
	...
	def test_ravel(self):
		...
		zero_dim_tensor = torch.tensor(123, device=self._device)
		flat0 = zero_dim_tensor.ravel()
		one_dim_tensor = torch.tensor([123], device=self._device)
		flat1 = zero_dim_tensor.ravel()
		nc_ones_tensor = torch.ones(10, device=self._device)[::2]
		...
		tensors = [
			torch.randn(5, 5, 5, 5, device=self._device),
			torch._empty_affine_quantized(
				[5, 5, 5, 5], scale=2, zero_point=3, dtype=torch.quint8, device=self._device
			),
		]
		_test_ravel(tensors, 625)

		tensors = [
			torch.randn(0, 2, 3, device=self._device),
			torch.randn(3, 0, 2, device=self._device),
			torch._empty_affine_quantized(
				[0, 2, 3], scale=2, zero_point=3, dtype=torch.quint8, device=self._device
			),
			torch._empty_affine_quantized(
				[3, 0, 2], scale=2, zero_point=3, dtype=torch.quint8, device=self._device
			),
		]
		_test_ravel(tensors, 0)

		tensors = [
			torch.randn(5, 5, device=self._device),
			torch._empty_affine_quantized(
				[5, 5], scale=2, zero_point=3, dtype=torch.quint8, device=self._device
			),
		]
	...
	def test_empty_reshape(self):
		x = torch.randn(0, 6, device=self._device)
	...
	def test_expand(self):
		tensor = torch.rand(1, 8, 1, device=self._device)
		tensor2 = torch.rand(5, device=self._device)
		template = torch.rand(4, 8, 5, device=self._device)
		...
		# test non-contiguous
		noncontig = torch.randn(5, 2, 1, 3, device=self._device)[:, 0]
		...
		self.assertEqual(
			torch.zeros(0, device=self._device).expand((0,)), torch.zeros(0, device=self._device)
		)
	...
	def test_reshape(self):
		...
		y = torch.randn(4, 4, 4, device=self._device)[:, 0, :]
		...
		s = torch.randn((), device=self._device)
		self.assertEqual(s.data_ptr(), s.reshape(()).data_ptr())
		self.assertEqual(s.reshape(-1).shape, (1,))
		self.assertRaises(RuntimeError, lambda: s.reshape(2))

		empty = torch.tensor([], device=self._device)
		...
		self.assertRaises(
			RuntimeError, lambda: x.reshape_as(torch.rand(10, device=self._device))
		)
	...
	def test_big_transpose(self):
		t = torch.rand(456, 789, device=self._device)
		t1 = t.t().contiguous()
		t2 = torch.from_numpy(t.cpu().numpy().transpose())
		self.assertEqual(t1, t2)

	def test_T(self):
		a = torch.randn(2, 3, 4, device=self._device)
		t1 = a.T
		t2 = a.permute(2, 1, 0)
		self.assertEqual(t2, t1)
		b = torch.randn(10, device=self._device)
	...
	def test_transposes(self):
		for dtype in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool):
			for op in ("T", "H", "mT", "mH", "adjoint"):
				...
				for shape in shapes:
					a = make_tensor(shape, device=self._device, dtype=dtype)
					t1 = getattr(a, op)
					if op == "adjoint":
						t1 = t1()
					t2 = a
	...
	def test_transposes_errors(self):
		for dtype in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool):
	...
	def test_python_types(self):
		a1 = torch.randn((1, 2), device=self._device, dtype=torch.float64)
		a2 = torch.randn((1, 2), device=self._device, dtype=float)
		self.assertEqual(a1.dtype, a2.dtype)

		b1 = torch.arange(10, 20, dtype=torch.int64, device=self._device)
		b2 = torch.arange(10, 20, dtype=int, device=self._device)
		self.assertEqual(b1.dtype, b2.dtype)

		c1 = torch.tensor([True, False], dtype=torch.bool, device=self._device)
		c2 = torch.tensor([True, False], dtype=bool, device=self._device)
		self.assertEqual(c1.dtype, c2.dtype)

	# TODO: is resize best put in test_view_ops?
	def test_resize_as_preserves_strides(self):
		x = torch.empty(2, 3).t()
		old_strides = x.stride()
		x.resize_as_(x)
		self.assertEqual(x.stride(), old_strides)

	def test_memory_format_resize_as(self):
		def test_helper(shape, memory_format, device):
			xc = torch.randn(shape, device=self._device).contiguous(
				memory_format=memory_format
			)
			flat = torch.randn(xc.numel(), device=self._device)
	...
	def test_memory_format_resize_(self):
		def test_helper(shape, numel, memory_format, device):
			flat = torch.randn(numel, device=self._device)
	...
	def test_transpose_invalid(self):
		for dtype in [torch.int64, torch.float, torch.complex128]:
			for fn in (torch.swapdims, torch.swapaxes, torch.transpose):
	...
	# TODO: OpInfo this
	def _test_atleast(self, torch_fn):
		# 0-dim
	...
	def test_atleast_gradient(self):
	...
	def test_broadcast_tensors(self):
		dtype = torch.float
		x0 = torch.randn(2, 1, 3, dtype=dtype, device=self._device)
		x1 = torch.randn(3, dtype=dtype, device=self._device)
		x2 = torch.randn(3, 1, dtype=dtype, device=self._device)
		expected_size = (2, 3, 3)
	...
	def test_broadcast_to(self):
		for dtype in all_types_and_complex_and(torch.half, torch.bool):
			...
			for s0, s1 in combinations(sizes, r=2):
				t = make_tensor(s0, dtype=dtype, device=self._device, low=-9, high=9)
				t_np = t.cpu().numpy()
	...
	def test_view(self):
		tensor = torch.rand(15, device=self._device)
		template = torch.rand(3, 5, device=self._device)
		...
		tensor = (
			torch.rand(4, 2, 5, 1, 6, 2, 9, 3, device=self._device)
			.transpose(-1, 2)
			.transpose(-2, 3)
		)
		...
		tensor = torch.empty(1, 1, device=self._device).expand(
			3, 4
	...
	def test_reshape_view_semantics(self):
		for dtype in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool):
			tensor = make_tensor((15, 4), dtype=dtype, device=self._device)
			target = (20, 3)
	...
	def test_contiguous(self):
		x = torch.randn(1, 16, 5, 5, device=self._device)
	...
	def test_tensor_split_sections(self):
		for dtype in all_types_and_complex_and(torch.half, torch.bool):
			...
			for input_size in input_sizes:
				a_base = make_tensor(input_size, dtype=dtype, device=self._device, low=-9, high=9)
	...
	#@onlyNativeDeviceTypes
	def test_tensor_split_errors(self):
		S = 10
		test_cases = [
		...
		for input_size, sections_or_indices, dim, err, err_msg, numpy_err in test_cases:
			a = torch.randn(input_size, device=self._device)
		...
		with self.assertRaisesRegex(
			...
			torch.tensor_split(torch.rand(S, device=self._device), torch.tensor(((1,),)), 0)

	def test_resize_all_dtypes_and_devices(self):
		shape = (2, 2)
		for dt in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool):
			x = torch.tensor([[1, 2], [3, 4], [5, 6]], dtype=dt, device=self._device)
			x.resize_(shape)
	...
	def test_resize_as_all_dtypes_and_devices(self):
		for dt in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool):
			x = torch.tensor([[1, 2], [3, 4], [5, 6]], dtype=dt, device=self._device)
			y = torch.tensor([[1, 2, 3], [4, 5, 6]], dtype=dt, device=self._device)
			x.resize_as_(y)
	...
	#@onlyNativeDeviceTypes
	def test_as_strided_overflow_storage_offset(self):
		t = torch.randn(2, 3, device=self._device)
		with self.assertRaisesRegex(
			RuntimeError, "Storage size calculation overflowed"
		):
	...
	#@onlyCPU
	#def test_crow_col_indices(self):
	#	crow_indices = (0, 1, 2)
	#	col_indices = (1, 0)
	#	values = (1, 2)
