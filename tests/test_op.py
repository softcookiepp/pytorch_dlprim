...
def test_all(device):
    ...
    print("where")
    test_fwd([([4,3,2],2),([4,3,1],-1),([4,1,2],-1)],lambda c,x,y:torch.where(c.to(torch.bool),x,y),device)
    print("masked_select broadcast")
    test_fwd([([4,3,2],-1),([4,1,1],2)],lambda x,m:torch.masked_select(x,m.to(torch.bool)),device)
    print("BCE Loss")
