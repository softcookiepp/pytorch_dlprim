...
def test_all(device):
	...
	print("Sum 3d")
	test_fwd_bwd([([4, 8, 9, 12], -1)],lambda x:torch.sum(x,dim=(0, 1, 2),keepdim=True),device)
	print("Sum 4d")
	test_fwd_bwd([([64, 8, 12, 12, 2], -1)],lambda x:torch.sum(x,dim=(0, 1, 3),keepdim=True),device)
	...
	test_fwd_bwd([([4,8,1,1],-1)],torch.nn.AdaptiveAvgPool2d((1,1)),device)
	print("MaxPool2d")
